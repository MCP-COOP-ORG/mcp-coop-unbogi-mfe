...
class GiftService {
    ...
    async sendGift(payload, senderId) {
        ...
        if (senderId === receiverId) {
            throw new https_1.HttpsError(contracts_1.ERROR_CODES.INVALID_ARGUMENT, contracts_1.GIFT_ERROR_MESSAGES.SELF_GIFT_FORBIDDEN);
        }
        const areConnected = await this.contactRepo.areUsersConnected(senderId, receiverId);
        if (!areConnected) {
            throw new https_1.HttpsError(contracts_1.ERROR_CODES.INVALID_ARGUMENT, contracts_1.GIFT_ERROR_MESSAGES.RECEIVER_NOT_IN_CONTACTS);
        }
        const holidaySnap = await this.holidayRepo.getHoliday(holidayId);
        if (!holidaySnap.exists) {
            throw new https_1.HttpsError(contracts_1.ERROR_CODES.NOT_FOUND, contracts_1.GIFT_ERROR_MESSAGES.HOLIDAY_NOT_FOUND);
        }
    ...
    async scratchGift(payload, callerId) {
        try {
            ...
            if (err.message === 'PERMISSION_DENIED') {
                throw new https_1.HttpsError(contracts_1.ERROR_CODES.PERMISSION_DENIED, contracts_1.GIFT_ERROR_MESSAGES.GIFT_ACCESS_DENIED);
            }
