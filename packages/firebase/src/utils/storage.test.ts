...
describe('Storage Utils (Unit)', () => {
  let mockBucketFile: any;
  let mockGetDownloadURL: any;

  beforeEach(() => {
    vi.clearAllMocks();
    const storageMock = admin.storage() as any;
    mockBucketFile = storageMock.bucket().file;
    mockGetDownloadURL = getDownloadURL;
  });
